...
export async function generateMetadata(
  ...
  if (!post) {
    return { title: 'Beitrag nicht gefunden — Itinerary of Türkiye', robots: { index: false } };
  }
  return {
    ...
    openGraph: {
      title: post.title,
      description: post.excerpt,
      url: `${SITE_URL}/de/blogs/${post.slug}`,
...
export default async function BlogPostPage(
  ...
  const articleJsonLd = {
    ...
    mainEntityOfPage: `${SITE_URL}/de/blogs/${post.slug}`,
  };
    ...
    <main style={{ fontFamily: "'Inter', system-ui, sans-serif", background: '#fffaf1', minHeight: '100vh' }}>
        ...
        <div className="post-hero-content">
          ...
          <div className="post-meta">
            ...
            {post.updatedISO && (
              <>
                <span>·</span>
                <span>Aktualisiert: <time dateTime={post.updatedISO}>{new Date(post.updatedISO + 'T00:00:00').toLocaleDateString('de-DE', { day: 'numeric', month: 'long', year: 'numeric' })}</time></span>
              </>
            )}
          </div>
        </div>
      ...
      {post.faq && post.faq.length > 0 && (
        <section className="post-faq" aria-labelledby="post-faq-title">
          <h2 id="post-faq-title">Häufig gestellte Fragen</h2>
          {post.faq.map((f, i) => (
